...
│ tool registry · LLM router · chat pipeline           │
└────────┬──────────────────────────────┬───────────────┘
         │ HTTPS                        │ HTTPS
┌────────▼────────────┐       ┌─────────▼──────────────┐
...
1. 토큰, IP별 호출 제한, 요청 크기를 검사합니다. LLM 미설정 서버는 챗봇 요청을 503으로 거부합니다(`/mcp`는 동작).
2. LLM이 필요한 공식 API 호출 1~4개를 계획합니다(도구+인자 JSON, 각각 Zod 스키마 검증). 유효한 계획이
   없으면 추측 조회 없이 안내 오류를 반환합니다.
3. 계획된 도구를 병렬로 조회합니다. 전부 실패하면 답변 생성을 하지 않고, 일부 실패는 답변 하단에 표시합니다.
4. LLM이 조회 자료 안의 내용만으로 자연어 답변을 작성하고 문장마다 `[자료 N]` 근거 번호를 인용합니다.
   서버는 인용 번호가 실존하는 자료를 가리키는지 검증하고 공식 조회 자료를 항상 병기합니다.
   답변 생성·인용 검증이 실패하면 조회 원문을 그대로 반환합니다(답변 증발 금지).

답변은 조회 자료 범위로 제한하도록 프롬프트로 강제하고 인용·원문 병기로 검증 가능하게 하지만, 문장 단위
원문 대조를 기계 검증하지는 않습니다. 중요한 판단은 병기된 원문으로 확인해야 하며, 원천 API 자체의
최신성·완전성도 별도 검토 대상입니다.
...
│   ├── llm-router.ts           스키마 검증 LLM 계획기 (호출 1~4개, 유일한 라우터)

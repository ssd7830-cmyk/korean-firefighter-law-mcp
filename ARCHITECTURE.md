...
2. LLM이 필요한 공식 API 호출 1~4개를 계획합니다(도구+인자 JSON, 각각 Zod 스키마 검증). 유효한 계획이
   없으면 추측 조회 없이 안내 오류를 반환합니다. 계획 내용은 stderr 로그로 관측할 수 있습니다.
3. 계획된 도구를 병렬로 조회합니다. 전부 실패하면 답변 생성을 하지 않고, 일부 실패는 답변 하단에 표시합니다.
4. LLM이 조회 자료 안의 내용만으로 구조화된 존댓말 답변(소제목·목록·굵은 수치)을 작성하고 주장·수치
   문장에 `[자료 N]` 근거 번호를 인용합니다. 자료가 부족하면 답변 대신 `[추가조회]` 신호를 내고, 서버는
   플래너를 재호출해 딱 1라운드 추가 조회 후 자료를 병합해 최종 답변을 받습니다(질문 1건당 LLM 최대
   4회·조회 최대 8건). 조회로도 없는 데이터는 공식 안내처(통계연보·대법원 종합법률정보 등)를 권합니다.
5. 서버는 답변에 실존 자료 번호 인용이 1개 이상 있고 비실존 번호 인용이 없는지 검증합니다. 통과한 답변은
   `answer`로, 공식 조회 원문은 별도 `sources` 필드로 반환하며 UI가 접이식으로 병기합니다.
...
└── web/chat-page.ts + chat-script.ts  단일 페이지 채팅 UI (마크업/동작 분리)
```
...
| 법령 | `search_fire_law` | 법령명 또는 본문 검색(전량 수신 후 소방·건축 소관 관련도순 재정렬), 약칭 지원 |
| 조문 | `get_fire_law_text` | 법령명과 선택 조 번호 또는 키워드(포함 조문만 추출) |
| 별표 | `get_fire_law_annex` | 법령명/MST, 별표 번호, 별표 내 키워드 |
| 판례 | `search_fire_precedents` | 법제처 판례 검색. 행정심판을 포함한다고 표시하지 않음 |
| 행정규칙 | `search_fire_admin_rules` | 고시·훈령, NFPC·NFTC 이름/본문(소방 소관 관련도순 재정렬) |
| 행정규칙 원문 | `get_fire_admin_rule_text` | 행정규칙 ID/명칭(토큰 일치 최단 이름 해석), 절 번호(줄 시작 매칭)·키워드(토큰 AND) |

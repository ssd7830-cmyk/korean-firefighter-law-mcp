...
결과만 표시하고, 키가 있으면 질문 라우팅과 관련 근거 선별에 LLM을 사용합니다. LLM이 고른 문장이 조회
원문의 정확한 부분문자열일 때만 표시하며 공식 조회 원문도 항상 함께 제공합니다.
...
- 기존 9개 검색 도구와 새 별표·행정규칙 원문 도구의 검증 범위는 [기준.md](기준.md)에 기록합니다.

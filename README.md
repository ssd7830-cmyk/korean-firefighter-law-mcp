...
기관 서버형은 `GET /` 웹 채팅, `POST /api/chat`, `POST /mcp`를 제공합니다. LLM 키가 없으면 공식 조회
결과만 표시하고, 키가 있으면 질문 라우팅과 요약에 LLM을 사용합니다. AI 요약 아래에는 공식 조회 원문을
항상 함께 표시합니다.
...
- LLM을 켜면 사용자 질문과 조회 자료가 선택한 외부 LLM 사업자로 전송됩니다. 기관의 개인정보·보안·기록물
  정책 검토 전에는 민감정보나 개인정보를 입력하지 말고, 필요하면 LLM 없는 조회 모드로 운영합니다.
...
- 9개 도구의 정부 API 실호출 성공 기록은 2026-08-13의 [기준.md](기준.md)에 있습니다. 현재 환경에는
  API 키가 없어 이번 코드 변경 뒤 실호출을 반복하지 못했으므로 배포 전 재검증이 필요합니다.
- 실제 Gemini·Claude·OpenAI 호출, Docker 이미지 빌드, ChatGPT/Claude 원격 MCP UI 연결은 현재 환경에서

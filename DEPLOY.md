...
예산과 갱신 일정을 정합니다. 질문 1건은 기본적으로 계획·답변 2회의 LLM 호출과 최대 4건의 공식 API
조회를 사용하며, 조회 자료가 부족하면 추가 조회 1라운드가 붙어 최대 4회 LLM 호출·8건 조회까지
사용할 수 있습니다.

답변은 공식 조회 자료에 있는 내용만으로 작성되며 주장·수치 문장에 `[자료 N]` 근거 번호를 인용합니다.
서버는 실존 자료 번호 인용이 1개 이상 있고 비실존 번호 인용이 없는지 검증하고, 공식 조회 원문을 응답의
`sources` 필드로 항상 함께 반환합니다(웹 화면은 접이식 표시). 이 결과 역시 기관의 공식 해석은 아닙니다.

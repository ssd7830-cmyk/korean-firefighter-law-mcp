...
- get_fire_admin_rule_text의 section은 키워드보다 절 번호("2.7")·조 번호("제10조")가 정확하다.
  이전 조회 자료에서 절·조 번호를 알게 됐으면 번호로 다시 조회한다 (키워드는 원문 표현과 다르면 빗나간다).
- 위험물의 지정수량·품명·유별 분류 기준은 위험물안전관리법 시행령 별표 1이 본체다. 별표 1의
  지정수량 표에는 물질명이 아니라 품명(제1석유류 등)만 있으므로, search_hazmat으로 품명을 확인하고
  get_fire_law_annex(lawName "위험물안전관리법 시행령", annex "1", query 품명)를 함께 계획한다.
  품명을 이미 알면 처음부터 품명으로 조회한다 (아세톤=제1석유류 수용성, 휘발유=제1석유류 비수용성 등).
